...
public class Face {
	...
	private FaceType mark = FaceType.VISIBLE;
	...
	public static Face create(Vertex[] vtxArray, int[] indices) {
		...
		for (int indice : indices) {
			HalfEdge he = new HalfEdge(vtxArray[indice], face);
			if (hePrev != null) {
				he.setPrev(hePrev);
				hePrev.setNext(he);
			} else {
	...
	public boolean isDeleted() {
		return mark == FaceType.DELETED;
	}
	
	public void setDeleted() {
		mark = FaceType.DELETED;
	}
	
	public boolean isNonConvex() {
		return mark == FaceType.NON_CONVEX;
	}
	
	public void setNonConvex() {
		mark = FaceType.NON_CONVEX;
	}

	public HalfEdge getHe0() {
		return he0;
	}
