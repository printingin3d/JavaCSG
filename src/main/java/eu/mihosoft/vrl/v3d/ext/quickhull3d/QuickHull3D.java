...
public class QuickHull3D {
	...
	private HalfEdge findHalfEdge(Vertex tail, Vertex head) {
		...
		for (Object element : faces) {
			HalfEdge he = ((Face) element).findEdge(tail, head);
			if (he != null) {
				return he;
			}
		}
		...
		try {
			...
			for (int i = 0; i < numf; i++) {
				...
				for (Integer index : indexList) {
					faceIndices[i][k++] = index.intValue();
				}
			}
	...
	protected void addNewFaces(FaceList newFaces, Vertex eyeVtx) {
		...
		for (Object element : horizon) {
			HalfEdge horizonHe = (HalfEdge) element;
